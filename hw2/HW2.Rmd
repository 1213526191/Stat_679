...
**Scores doesn't follow the normal distribution. Some values appear more frequently.**
...
**Values of SES are around 6.4 instead of 5 which means higher values appeals more frequently than lower ones.**
...
**There seems to be a linear relationship between SES and Score.**
...
print(answer4)
```
**The total number of students are `r N` and schools are `r J`.** 
...
1. Generally speaking, the higher SES, the higher Score.
2. The linear relationship between SES and scores is more signicicant for schools with high SES.
...
\begin{aligned}
  y_i 
  &= (\beta_0 + \alpha_{j[i]}) + \beta_1x_i + \epsilon_i\\
  &\alpha \sim N(0, \sigma_A)\\
  &\epsilon \sim N(0, \sigma)
\end{aligned}
$$

# Q9
```{r Q9, message = FALSE, results="hide"}
set.seed(12345)
sigmaA = 2
...
**The estimaters are almost the same.**

# Q11

```{r Q11, message = FALSE, results="hide"}
...
**The credible regions are almost the same.**

# Q12
```{r Q12, message = FALSE, results="hide"}
fit3 = stan(file="hw02_2.stan",seed=23456, data = hw02_data)
fit3.extract = rstan::extract(fit3)
beta3 = summary(fit3, pars="beta")$summary
alpha3 = summary(fit3, pars="alpha")$summary
sigma3 = summary(fit3, pars="sigma")$summary
sigmaA3 = summary(fit3, pars="sigmaA")$summary
answer12 = tibble(
  parameter = c("stan1_beta0", "stan1_beta1", "stan3_beta0", "stan3_beta1"),
  "2.5%" = c(beta[1,4], beta[2,4], beta3[1,4], beta3[2,4]),
  "97.5%" = c(beta[1,8], beta[2,8], beta3[1,8], beta3[2,8])
)
```
...
1. The lower bound of intercept in model with Cauchy distribution decrease a bit.
2. The lower bound of slope in model with Cauchy distribution increase a bit.
...
**Almost all the estimated school effects are the same.**

# Q14

```{r Q14, message = FALSE, results="hide"}
...
**From the plot, we can see that the confidence intervals of lmer is shorter than stan when predicting the mean score. However, in case of predicting for all students in the school with an SES value of 7, though the confidence intervals of lmer is still shorter, the difference between is small.**

# Q15

```{r Q15, message = FALSE, results="hide"}
da_sub = da[which(da$School %in% school_label$label),] %>%
  arrange(School) 
da_sub3 = da_sub %>%
  group_by(School)%>%
...
**When predicting one student, the confidence intervals of stan is shorter. But about SchoolC, the difference between predictions of two methods are significant larger than other four school.**
...
**There exists a linear patterm between residuals and fitted values. For further analysis, I want to fit a linear regression between Score and 1/SES.**